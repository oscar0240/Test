...
//그래픽 카드 관련된 정보를 IDXGIAdapter인터페이스를 얻어옴
void D3D::SetGpuInfo()
{
	HRESULT hr;
...
/*
DirectX 11의 처리 흐름도(위에 내용의 흐름도)
1.디바이스와 스왑체인 생성(CreateSwapChainAndDevice)(필요에 따라 깊이/스텐실 버퍼 생성)
2.스왑 체인의 백 버퍼를 Direct3D의 렌더타켓으로 설정(CreateBackBuffer)(필요에 따라 깊이/스텐실 버퍼 생성)

이렇게 화면을 구성하고 어플리케이션을 실행하고 필요한 처리를 수행한다.
일정간격으로 화면을 렌더링하고 스왑체인으로 화면을 갱신하다.
